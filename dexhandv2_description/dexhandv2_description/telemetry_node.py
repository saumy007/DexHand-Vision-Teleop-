...
class DexHandTelemetryNode(Node):
    def __init__(self):
        ...
        # 2. Resolve absolute path to the MediaPipe model asset
        # This points directly to your package source folder so it always finds it
        model_path = os.path.expanduser('~/dex_hands_project/dexhandv2_description/dexhandv2_description/hand_landmarker.task')
        
        if not os.path.exists(model_path):
            self.get_logger().error(f"CRITICAL: Model file not found at {model_path}. Please download it there!")
            raise FileNotFoundError(f"Missing asset: {model_path}")
        ...
        # 5. Set Timer Loop (0.05 seconds = 20 Hz)
    ...
    def camera_loop_callback(self):
        ...
        if detection_result.hand_landmarks:
            ...
            self.get_logger().info(f"Published 21 joints. Index Flexor: {joint_angles['R_Index_Flexor']:.3f} rad")
